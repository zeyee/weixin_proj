...
function wxSearchInput(e) {
  ...
  // 寻找提示值 
  var tipKeys = [];
  // console.log('运行到wxSerrchInput')
  // console.log(inputValue.length)
  // console.log(inputValue)
  // console.log(__tipKeys.length)
  console.log(__tipKeys[1])
  console.log(__tipKeys[1].indexOf(inputValue))
  if (inputValue && inputValue.length > 0) {
    // console.log("If 条件成功")
    for (var i = 0; i < __tipKeys.length; i++) {
      var mindKey = __tipKeys[i];
      // console.log(typeof mindKey)
      // console.log(typeof inputValue)
      // console.log(mindKey.indexOf(inputValue))
      // 包含字符串
...
function getHisKeys() {
  var value = [];
  try {
    value = wx.getStorageSync('wxFileSearchHisKeys')
    if (value) {
...
function wxSearchAddHisKey(inputValue) {
  ...
  var value = wx.getStorageSync('wxFileSearchHisKeys');
  if (value) {
    if (value.indexOf(inputValue) < 0) {
      value.unshift(inputValue);
    }
    wx.setStorage({
      key: "wxFileSearchHisKeys",
      data: value,
      success: function () {
        getHisKeys(__that);
      }
